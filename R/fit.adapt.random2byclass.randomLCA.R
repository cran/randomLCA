fit.adapt.random2byclass.randomLCA <- function(outcomes,freq,nclass=2,initoutcomep,initclassp,initlambdacoef,initltaucoef,
    ...
    calclikelihood <- function(classx,outcomex,lambdacoef,ltaucoef,momentdata,gh,updatemoments=FALSE,calcfitted=FALSE) {
...
#			print("class")
#			print(iclass)
#			print(classp2[iclass])
#			print(outcomex[iclass,])
#			print(lambdacoef[iclass,])
#			print(ltaucoef[iclass])
			
			result <- .Call("bernoulliprobrandom2",outcomes,outcomex[iclass,],lambdacoef[iclass,],
				ltaucoef[iclass],gh,momentdata[,((iclass-1)*(2+nlevel2*3)+1):(iclass*(2+nlevel2*3))],
...
# add extra column to classp
    ...
    outcomex <- matrix(optim.fit$estimate[nclass:(nclass+nlevel1*nlevel2*nclass-1)],nrow=nclass)
