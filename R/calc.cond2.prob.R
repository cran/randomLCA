...
# need to integrate over tau
	...
	outcome <- factor(rep(rep(1:object$blocksize,times=nblocks),length(conditionalp)))
	class <- factor(rep(rep(1:object$nclass,each=object$blocksize*nblocks),length(conditionalp)))
	block <- factor(rep(rep(1:nblocks,each=object$blocksize,times=object$nclass),length(conditionalp)))
