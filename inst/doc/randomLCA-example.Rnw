...
<<>>=
classprobs <- postClassProbs(myocardial.lca2)
@ 
...
<<results=tex>>=
print(xtable(classprobs, digits = c(0,0,0,0,0,0,3,3),caption="Class Probabilities.",
...
dentistry.lca2random <- randomLCA(dentistry[,1:5],freq=dentistry$freq,
	nclass=2,random=TRUE,quadpoints=61,probit=TRUE)
@ 
...
The BIC increases to \Sexpr{round(BIC(dentistry.lca2random2),1)}. It is not surprising that this model isn't an improvement, there are now 21 parameters fitted to 32 observations. This also may give problems with the fitting algorithm so the number of quadrature points is increased to 41.
