...
<<>>=
bic.data <- data.frame(classes=1:3,bic=c(myocardial.lca1$bic,
	myocardial.lca2$bic,myocardial.lca3$bic))
@ 
...
The outcome probabilities can then be obtained, and are shown in Table~\ref{tab:outcomep1}.
<<>>=
outcomep.data <- myocardial.lca2$outcomep
# I should do this in randomLCA
rownames(outcomep.data) <- c("Class 1","Class 2")
colnames(outcomep.data) <- names(myocardial)[1:4]
@ 
...
This gives some interesting information. In Class 2, those without myocardial infarction, will have abscence of Q.wave but in those with myocardial infarction it will only be present in 76.7\% of those with myocardial infarction. The class probabilities can be obtained as \texttt{myocardial.lca2\$classp} of \Sexpr{round(myocardial.lca2$classp[1],2)} and \Sexpr{round(myocardial.lca2$classp[2],2)} for Class 1 and 2 respectively.
