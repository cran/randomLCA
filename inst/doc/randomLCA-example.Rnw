...
This gives some interesting information. In Class 2, those without myocardial infarction, will have abscence of Q.wave but in those with myocardial infarction it will only be present in 76.7\%. The class probabilities can be obtained as \texttt{myocardial.lca2\$classp} of \Sexpr{round(myocardial.lca2$classp[1],2)} and \Sexpr{round(myocardial.lca2$classp[2],2)} for Class 1 and 2 respectively.
...
probs <- data.frame(tpr=probs[[2]][,1],fpr=probs[[1]][,1])
@ 
...
<<fig=TRUE,width=7,height=5>>=
trellis.par.set(col.whitebg())
print(plot(tpr~fpr,type="p",xlab="False Positive Rate\n(1-Specificity)",ylab="True Positive Rate (Sensitivity)",data=probs))
@  
  \caption{True and False Positive Rates by Dentist.}
  \label{fig:truefalse}
\end{figure}

\subsection{Latent Class with Random Effects}
...
This increases the BIC to \Sexpr{round(BIC(dentistry.lca2random1),1)}, and is the 2LCR model obtained by Qu, Tan and Kutner (1996). It appears that the simpler model is more appropriate.
